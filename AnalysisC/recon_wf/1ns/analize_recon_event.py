...
# data=np.load(path+'recon1ns61999.npz')
# rec=data['rec']
# WFs=data['WFs']
# recon_WFs=data['recon_WFs']
...
plt.figure()
plt.hist(np.sum(np.sum(rec['h'], axis=2), axis=1),  bins=np.arange(750)*2-1, histtype='step', label='all')
plt.hist(np.sum(np.sum(s_rec['h'], axis=2), axis=1),  bins=np.arange(750)*2-1, histtype='step', label='slow')
plt.hist(np.sum(np.sum(f_rec['h'], axis=2), axis=1), bins=np.arange(750)*2-1, histtype='step', label='fast')
...
plt.figure()
plt.hist(np.argmax(np.sum(rec['h'], axis=1), axis=1), bins=np.arange(20)-0.5)
#
# plt.figure()
...
#
# plt.figure()
# plt.title('Distribution od the number of PEs on one PMT that coused a trigger')
# plt.hist(np.amax(np.sum(rec['h'][:,:10,:], axis=1), axis=1), bins=100, histtype='step')
# plt.hist(np.amax(np.sum(s_rec['h'][:,:10,:], axis=1), axis=1), bins=100, histtype='step')
