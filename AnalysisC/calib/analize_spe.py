...
h_heights, bins, pat=ax3.hist(rec['height'], bins=100, label='height', range=[0,500], histtype='step')
heights=0.5*(bins[1:]+bins[:-1])
ax3.axvline(height_cut, ymin=0, ymax=1)
ax3.set_yscale('log')
ax3.legend()

h_area, bins, pat=ax4.hist(rec['area'], bins=100, label='area', range=[-2700, 10000], histtype='step')
areas=0.5*(bins[:-1]+bins[1:])
rng=np.nonzero(np.logical_or(np.logical_and(areas>-500, areas<1000), np.logical_and(areas>3000, areas<10000)))[0]
ax4.set_yscale('log')
p=[4566.3385835,   401.26508103,   46.80310819,  404.40993119,  4700,
  1500]


bounds_up=[1e5, 1e5, 1000, 1000, 20000, 1e5]
bounds_dn=[0,0,-1000,0,4500, 0]
