...
def L(p):
    ...
    B=multiprocessing.Process(target=make_l, args=(lb, HB, spectraB, covB, leftB, rightB, XcovB, 1, 0, 122, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], eta[0]))
    A=multiprocessing.Process(target=make_l, args=(la, H, spectra, cov, left, right, Xcov, 0, 1, 122, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], eta[0]))
...
def make_l(l, H, spectra, cov, left, right, Xcov, x1, x2, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, eta):
    S, Sspectra, Scov=Sim_fit(x1, x2, left, right, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, eta, np.arange(np.shape(spectra)[0]))
    if np.any(S<0):
