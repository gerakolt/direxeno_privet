...
def make_3D(t, N, F, Tf, Ts, R, a, eta, Q, T, St, dS, PEs, r_mash, V_mash):
    dt=t[1]-t[0]
...
def make_3D2(t, N, F, Tf, Ts, R, a, eta, Q, T, St, dS, PEs, r_mash, V_mash, x):
    dt=t[1]-t[0]
    r=np.arange(t[0], t[-1]+dt, dt/100)
    dr=r[1]-r[0]
        ...
        for j in range(i+1,6):
            for p1, pe1 in enumerate(PEs):
                for p2, pe2 in enumerate(PEs):
                    ...
                    print(c)
                    if np.abs(c)<=0.1:
                        cov[np.argmin(np.abs(c-x)), k]+=S[p1,i]*S[p2,j]
            k+=1
    ...
    for i in range(1, 100):
        P[i]=np.sum(np.sum(B[:,:,i:,:]*P1[:len(t)-i,:]*pois, axis=3), axis=2)
    return np.transpose(P, (1,0,2)), S, cov
...
def Sim(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs):
    ...
    for i in range(N_events):
        ...
        pmt_hit=whichPMT(v, us, mid, rt, up)
    ...
    for i in range(5):
        ...
        for j in range(i+1,6):
            ...
            Mj=np.mean(Sj)
            cov[:,k]=Si*Sj/(Mi*Mj)-Si/Mi-Sj/Mj+1
            k+=1
...
def Sim2(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu):
    f=np.sum(make_recomb(np.arange(1000*20)/100, a, eta).reshape(1000,20), axis=1)
    if np.abs(np.sum(f)-1)>1e-4:
        print('In recombenation model:')
        print('sum(f):', np.sum(f))
