...
def make_v(N, mu, type):
    ...
    while len(V)<=N:
        ...
        u=o+np.amin([x1,x2])*(v-o)
        d=np.sqrt(np.sum((v-u)**2))
        # print(d,1-np.exp(-d/mu)/mu, np.exp(-d/mu)/mu)
        if 1==np.random.choice([0,1], size=1,  p=[1-0.1*np.exp(-d/mu)/mu, 0.1*np.exp(-d/mu)/mu]):
            V=np.vstack((V,v))
    ...
    # ax.scatter(V[1:,0], V[1:,1], V[1:,2], color='k', marker='.')
    # plt.show()
