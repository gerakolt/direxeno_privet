...
q=[]
counter=-1
for NQ in np.linspace(40,60,20):
    counter+=1
    def L(p):
        [ts, T, St, Spe, Mpe]=p
        if St<=0:
        ...
        # l4=0
        # l3=0
        print('In L' ,-(l1/len(J)+l4/len(x_area[area_rng])+l3/len(np.nonzero(spec_y>0)[0])), counter)
        return -(l1/len(J)+l4/len(x_area[area_rng])+l3/len(np.nonzero(spec_y>0)[0]))

    if counter==0:
        p0=np.array([45, 43, 0.7, 0.77, 2000])
    else:
        p0=p2.x
    p1=minimize(L, p0, method='Nelder-Mead', options={'disp':True, 'maxfev':10000})
    p2=minimize(L, p1.x, method='Nelder-Mead', options={'disp':True, 'maxfev':10000})
    prm.append(NQ)
    q.append(L(p2.x))
np.savez(path+'NQs', prm=prm, q=q)
