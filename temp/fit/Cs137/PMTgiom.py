...
def make_mash(pmts):
    ...
    r=r3**(1/3)
    v=np.vstack((r*np.sin(np.arccos(costheta))*np.cos(phi), np.vstack((r*np.sin(np.arccos(costheta))*np.sin(phi), r*costheta)))).T
    ds=make_dS(v, mid, rt, up)
    D=(np.sum(v**2, axis=0)-2*np.matmul(R,v)).T+np.sum(R**2, axis=1)
    for i in range(len(R)):
        dS[i]+=np.sum(ds[np.argmin(D, axis=1)==i])/N
        V[np.argmin(D)]+=len(np.nonzero(np.argmin(D, axis=1)==i))/N
