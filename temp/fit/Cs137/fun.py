...
def Sim(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu, x1, x2, r_mash, V_mash, l, r):
    ...
    # d=d[np.logical_and(np.sum(np.sum(d, axis=2), axis=1)>l, np.sum(np.sum(d, axis=2), axis=1)<r)]
    ...
    for i in range(5):
        ...
        for j in range(i+1,6):
            ...
            Mj=np.mean(Sj)
            cov[:,k], bins=np.histogram((Si-Mi)*(Sj-Mj)/(Mi*Mj), bins=5, range=[-0.1,0.1])
            k+=1
    ...
    return H/N_events, G/N_events, spectrum, spectra, Gtrp/N_events, Gsng/N_events, GRtrp/N_events, GRsng/N_events, cov, 0.5*(bins[1:]+bins[:-1])
