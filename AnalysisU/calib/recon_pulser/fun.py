...
def Recon_wf(WF, wf_origin, height_cut, dh3_cut, SPE, Init, L, R, th):
    t=[]
    areas=[]
    DCareas=[]
    ...
    while len(maxis)>0 and counter<1000:
        ...
        for i in range(left+10, maxi+10):
            ...
            if chi2<Chi2:
                Chi2=chi2
                I=i
        if np.amin(wf[left:np.argmin(spe)+1])/(np.amin(spe))>th:
            t.append(I)
        spe=np.roll(SPE, I-np.argmin(SPE))
        if left>=np.argmin(spe):
            break
        if np.argmin(spe)>L and np.argmin(spe)<R:
            areas.append(np.amin(wf[left:np.argmin(spe)+1])/(np.amin(spe)))
        else:
            DCareas.append(np.amin(wf[left:np.argmin(spe)+1])/(np.amin(spe)))
        if np.amin(spe)<np.amin(wf[left:np.argmin(spe)+1]):
            spe=np.amin(wf[left:np.argmin(spe)+1])/(np.amin(spe))*spe
        # if np.amin(spe)>np.amin(wf[left:np.argmin(spe)+1]):
        #     spe=np.amin(wf[left:np.argmin(spe)+1])/(np.amin(spe))*spe
        spe_dif=np.roll(SPE_dif, I-np.argmin(SPE_dif))
        ...
        # plt.plot(x, spe, 'b-',label='SPE Template', linewidth=7)
        # plt.plot(x, temp, 'g--',label='Residual Signal', linewidth=7)
        # plt.plot(x, recon_wf, 'r--', label='Reconstructed Signal', linewidth=5)
        # plt.fill_between(x, wf_origin, 0, color='k', alpha=0.5, label='Original Signal')
        # plt.axvline(L/5, 0, 1, color='k')
        # plt.axvline(R/5, 0, 1, color='k')
        ...
        maxis=Init+np.nonzero(np.logical_and(wf[Init:990]<-height_cut, np.logical_and(wf[Init:990]<np.roll(wf[Init:990], -1), wf[Init:990]<np.roll(wf[Init:990], 1))))[0]
        maxis=maxis[np.logical_and((wf[maxis]-wf[maxis-3])/wf[maxis]<dh3_cut, (wf[maxis]-wf[maxis+3])/wf[maxis]<dh3_cut)]
    Areas, Abins=np.histogram(areas, bins=np.arange(61)/20)
    DCAreas, Abins=np.histogram(DCareas, bins=np.arange(61)/20)
    return np.histogram(t, bins=np.arange(1001))[0], recon_wf, Areas, DCAreas, Abins
