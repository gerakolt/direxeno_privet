...
def Sim_fit(x1, x2, left, right, gamma, Q, T ,St, W, std, nLXe, sigma_smr, mu, R, a, F, Tf, Ts, Bins, bins):
    ...
    Ret=np.array(ret)
    # plt.figure()
    # plt.plot(np.arange(100), a/(1+a*np.arange(100)**2))
    # plt.show()
    plt.figure()
    plt.plot(np.arange(100), np.mean(np.sum(Ret, axis=-1), axis=0), 'k.-')
    plt.show()
    s=np.sum(Ret, axis=1)
    up=s[:,0]+s[:,1]
    dn=s[:,-1]+s[:,-2]+s[:,-3]
...
p=([2.33833932e-01, 1.33639204e-01, 1.36017992e-01, 1.79787452e-01,
 1.80894889e-01, 1.90115977e-01, 42, 42, 42, 42, 42, 42, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8,
 3.82220038e-01, 2.00141610e-01, 4.26199366e+01, 5.25757731e+02, 1.7, 1, 0.1, 0.54,  0.001, 0.02, 2, 32])

Q, T, St, W, std, Nbg, nLXe, sigma_smr, mu, R, a, F, Tf, Ts=make_glob_array(p)
spectrum, spectra, g, h=Sim_fit(0, 1, 0, 400, 122, Q, T, St, W, std, nLXe, sigma_smr, mu, R, a, F, Tf, Ts, np.arange(500), np.arange(100))
