...
def Sim_fit(x1, x2, left, right, gamma, Q, T ,St, W, std, nLXe, sigma_smr, mu, R, a, F, Tf, Ts, Bins, bins):
    ...
    for i in range(np.shape(Ret)[2]):
        G[0,:,i]=np.histogram(np.sum(Ret[:,0,i,:], axis=1), bins=np.arange((np.shape(G)[1]+1))-0.5)[0]
        G[1,:,i]=np.histogram(np.sum(Ret[:,1,i,:], axis=1), bins=np.arange((np.shape(G)[1]+1))-0.5)[0]
        G[2,:,i]=np.histogram(np.sum(Ret[:,2,i,:], axis=1), bins=np.arange((np.shape(G)[1]+1))-0.5)[0]
        G[3,:,i]=np.histogram(np.sum(Ret[:,3,i,:], axis=1), bins=np.arange((np.shape(G)[1]+1))-0.5)[0]
        G[4,:,i]=np.histogram(np.sum(Ret[:,4,i,:], axis=1), bins=np.arange((np.shape(G)[1]+1))-0.5)[0]
        for j in range(len(Q)):
            H[:,i,j]=np.histogram(Ret[:,0,i,j], bins=np.arange((np.shape(H)[0]+1))-0.5)[0]

    spectrum=np.histogram(np.sum(np.sum(Ret[:,0], axis=1), axis=1), bins=Bins)[0]
    for i in range(len(Q)):
        spectra[:,i]=np.histogram(np.sum(Ret[:,0,:,i], axis=1), bins=bins)[0]
...
def make_d(iter):
    ...
    for j in range(len(Q)):
        ...
        ind_ex1=np.logical_and(PE_extrct>=recomb, np.isin(PE_extrct, np.nonzero(sng_ind==0)[0]))
        ind_ex3=np.logical_and(PE_extrct>=recomb, np.isin(PE_extrct, np.nonzero(sng_ind==1)[0]))
        ind_recomb1=np.logical_and(PE_extrct<recomb, np.isin(PE_extrct, np.nonzero(sng_ind==0)[0]))
        ind_recomb3=np.logical_and(PE_extrct<recomb, np.isin(PE_extrct, np.nonzero(sng_ind==1)[0]))
        h[0,:,j]=np.histogram(tj, bins=np.arange(np.shape(h)[1]+1)-0.5)[0]
        h[1,:,j]=np.histogram(tj[ind_ex1], bins=np.arange(np.shape(h)[1]+1)-0.5)[0]
        h[2,:,j]=np.histogram(tj[ind_ex3], bins=np.arange(np.shape(h)[1]+1)-0.5)[0]
        h[3,:,j]=np.histogram(tj[ind_recomb1], bins=np.arange(np.shape(h)[1]+1)-0.5)[0]
        h[4,:,j]=np.histogram(tj[ind_recomb3], bins=np.arange(np.shape(h)[1]+1)-0.5)[0]
        if np.any(h[0,:,j]>0):
            t0[j]=np.amin(np.nonzero(h[0,:,j]>0)[0])

    for j in range(len(Q)):
        h[0,:,j]=np.roll(h[0,:,j], -int(np.amin(t0)))
        h[1,:,j]=np.roll(h[1,:,j], -int(np.amin(t0)))
        h[2,:,j]=np.roll(h[2,:,j], -int(np.amin(t0)))
        h[3,:,j]=np.roll(h[3,:,j], -int(np.amin(t0)))
        h[4,:,j]=np.roll(h[4,:,j], -int(np.amin(t0)))
    return h[:,:100]
