...
pmts=[0,1,4,7,8,14]
note='Only spectrum, one gamma, variation in w, bins of 20PEs'
...
counter=0
ls=[]
ps=np.zeros((15000, 3*len(pmts)+7))
t0=time.time()
...
def L(p):
    ...
    #B=multiprocessing.Process(target=make_l, args=(lb, HB, spectraB, covB, leftB, rightB, XcovB, 1, 0, gamma, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0]))
