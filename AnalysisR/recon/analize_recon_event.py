...
for i in range(len(pmts)):
    np.ravel(ax)[i].hist(np.sum(rec['h'][:,:,i], axis=1),  bins=np.arange(100), histtype='step', label='After\n up-dn cut\n PMT{}'.format(i), linewidth=3)
    np.ravel(ax)[i].hist(np.sum(rec0['h'][:,:,i], axis=1),  bins=np.arange(100), histtype='step', label='Before\n up-dn cut', linewidth=3)
    np.ravel(ax)[i].legend(fontsize=15)
