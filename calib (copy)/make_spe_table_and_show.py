...
while id<1e6:
    ...
    for peak in find_peaks(np.reshape(wf, (len(wf), 1)), np.array([blw]), pmts):
        if peak.init10-trig>left and peak.init10-trig<right and peak.maxi-peak.init10>d_cut and peak.height>height_cut:

            fig=plt.figure()
            ax=fig.add_subplot(211)
            ax.plot(np.roll(wf_copy, 200-peak.init10), 'k.')
            ax=fig.add_subplot(212)
            ax.plot(np.mean(SPE[:j], axis=0), 'r.-')
            plt.show()
