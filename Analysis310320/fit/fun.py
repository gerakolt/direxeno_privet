...
def Model2(NQs, F, Tf, Ts, Sts, Ps):
    ...
    for i, P in enumerate(Ps):
        ...
        temporal[:,:,i]=temporal[:,:,i]/np.sum(temporal[:,:,i], axis=0)
    return temporal


def Model3(NQs, R, F, Tf, Ts, Sts, Ps):
    T=32
    n=15
    t=np.arange(2000)/5
    dt=t[1]-t[0]
    ...
    Ms=np.zeros((2*n, len(t), len(Ps)))
    for i in range(len(Ps)):
        m=(NQs[i]*(R[i]*dt*np.exp(-0.5*(t-T)**2/Sts[i]**2)/np.sqrt(2*np.pi*Sts[i]**2)+(1-R[i])*(F*Int(t,Tf,T,Sts[i])+(1-F)*Int(t,Ts,T,Sts[i]))))
        Ms[:,:,i]=(poisson.pmf(np.floor(I/len(t)), m[I%len(t)]).reshape((2*n, len(t))))
    T0, P0=make_T0(Ms[0,:1000,:])
    temporal=np.zeros((n, len(T0), len(Ps)))
    for i, P in enumerate(Ps):
...
def Model4(NQs, R, F, D, Tf, Ts, Sts, Ps):
    T=32
    n=15
    t=np.arange(2000)/5
    dt=t[1]-t[0]
    ...
    Ms=np.zeros((2*n, len(t), len(Ps)))
    for i in range(len(Ps)):
        m=(NQs[i]*(R[i]*dt*np.exp(-0.5*(t-T)**2/Sts[i]**2)/np.sqrt(2*np.pi*Sts[i]**2)+(1-R[i])*(F*Int(t,Tf,T,Sts[i])+D*Int(t,Ts,T,Sts[i]))))
        Ms[:,:,i]=(poisson.pmf(np.floor(I/len(t)), m[I%len(t)]).reshape((2*n, len(t))))
    T0, P0=make_T0(Ms[0,:1000,:])
    temporal=np.zeros((n, len(T0), len(Ps)))
    for i, P in enumerate(Ps):
