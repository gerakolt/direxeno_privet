...
while True:
    ...
    for i, pmt in enumerate(pmts):
        ...
        rec[j]['h'][:,i]=h
    for i, pmt in enumerate(pmts):
        rec[j]['h'][:,i]=np.roll(rec[j]['h'][:,i], 0)
        rec[j]['init']=init
    j+=1
    id+=1
    if j==len(rec):
        np.savez(path+'EventRecon/try{}'.format(id-1), rec=rec, WFs=WFs, recon_WFs=recon_WFs, pmts=pmts)
        WFs=np.zeros((len(pmts), 1000))
        recon_WFs=np.zeros((len(pmts), 1000))
        j=0
np.savez(path+'EventRecon/try{}'.format(id-1), rec=rec[:j-1], WFs=WFs, recon_WFs=recon_WFs, pmts=pmts)
