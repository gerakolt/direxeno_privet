...
def L(p):
    global counter
    counter+=1
    [NQ1, Mpad, Spad, Mpe, Spe, a_pad, a_spe, a_dpe, p01, a_spec1, BG_r]=p
    ...
    if np.shape(P)==():
        return 1e10*P
    h_spec=model_spec(ns, NQ1, P, a_spec1)
...
p=[3.55555088e+02, 1.13670852e-03, 2.99964487e+02,
 9.95412916e+02, 6.68365996e-01, 1.15909739e+03, 3.09903429e+02,
 2.59317734e+02, 3.48880387e-01, 6.93871292e+03, 1.44701300e+00]

p=minimize(L, p, method='Nelder-Mead', options={'disp':True, 'maxfev':10000})
print(p.x)
[NQ, Mpad, Spad, Mpe, Spe, a_pad, a_spe, a_dpe, p01, a_spec, BG_r]=p.x
P=make_P(Spe, p01)
h_spec=model_spec(ns, NQ1, P, a_spec1)
...
ax1.plot(ns, a_spec1*poisson.pmf(ns, NQ1), 'g.-')
