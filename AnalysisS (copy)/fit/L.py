...
def L(p, pmt, q, ps, ls, ID):
    ...
    A=multiprocessing.Process(target=make_l, args=(len(ls)+1, la, HA, spectrumA, spectraA, TA, binsSpec, bins, covB, left, right, 0, 1, gamma, Q, W, mu, nLXe, sigma_smr))
...
def make_l(i, l, H, spectrum, spectra, T, Bins, bins, cov, left, right, x1, x2, gamma, Q, W, mu, nLXe, sigma_smr):
    np.random.seed(int(i*time.time()%2**32))
    Sspectrum, Sspectra, Scov=Sim_fit(x1, x2, left, right, gamma, Q, W, mu, nLXe, sigma_smr, binsSpec, bins)
