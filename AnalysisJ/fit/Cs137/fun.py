...
def make_3D(t,dt, N, F, Tf, Ts, R, a, eta, Q, T, St, dS, PEs, r_mash, V_mash, Xcov):
    r=np.arange(0,200,1/100)
    dr=r[1]-r[0]
    m=np.sum(((1-R)*Promt(r, F, Tf, Ts, T, St)+R*(1-eta)*Recomb(r, F, Tf, Ts, T, St,a,eta)).reshape(200, 100, len(T)), axis=1)
    model=np.zeros((len(t), len(T)))
    for i in range(len(t)):
        model[i]=np.sum(m[t[i]:t[i]+dt[i]], axis=0)
    ...
    for i in range(1, len(t[t<100])):
        ...
        P[i]=np.sum(np.sum(B[:,:,i:,:]*P1[:len(t)-i,:]*pois/dt[i], axis=3), axis=2)
...
def Sim(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu, x1, x2, r_mash):
    ...
    v=make_v(N_events, mu, x1, x2, r_mash)
...
def Sim_fit(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu, x1, x2, r_mash, left, right):
    f=np.sum(make_recomb(np.arange(1000*20)/100, a, eta).reshape(1000,20), axis=1)
    i=2
