...
def make_l(L, N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu, x1, x2, left, right, H, spectra, cov):
    l=0
    S, Sspectra, Scov=Sim_fit(N, F, Tf, Ts, R, a, eta, Q, T, St, PEs, mu, x1, x2, left, right)
...
def L(p):
    ...
    l_co57=multiprocessing.Value('d', 0.0)
    l_co57B=multiprocessing.Value('d', 0.0)

    Q=multiprocessing.Array(rec['Q'][0])

    co57=multiprocessing.Process(target=make_l, args=(l_co57, rec['N'][0], rec['F'][0], rec['Tf'][0], rec['Ts'][0], rec['R'][0], rec['a'][0],
                                                                        rec['eta'][0], rec['Q'][0], rec['T'][0], rec['St'][0], PEs, rec[0]['mu'], 0, 1, left, right, H, spectra, cov))
    co57B=multiprocessing.Process(target=make_l, args=(l_co57B, rec['N'][0], rec['F'][0], rec['Tf'][0], rec['Ts'][0], rec['R'][0], rec['a'][0],
                                                                        rec['eta'][0], rec['Q'][0], rec['T'][0], rec['St'][0], PEs, rec[0]['mu'], 1, 0, left, right, HB, spectraB, covB))
    ...
    Rec[counter-1]=rec[0]
    ls.append(-l)
    np.savez('Rec_'+source, Rec=Rec, ls=ls, time=time.time()-start_time)
    return -l
