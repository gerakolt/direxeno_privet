...
def Sim(NQ, T, Strig, R, F, Tf, Ts, St):
    ...
    for i in range(N_events):
        ...
        for j in range(len(NQ)):
            ...
            ch0=np.random.choice(15, size=200, replace=True, p=np.append(1-q0/(1-q0), q0**np.arange(1, 15)))
            ...
            for k in np.nonzero(h>0)[0]:
                a=np.random.normal(h[k], np.sqrt(h[k]*Spe**2))
                if a<a0:
                    h[k]=0
                elif a>a0 and a<1.5:
                    h[k]=1
    ...
    G_spectrum=np.histogram(np.sum(np.sum(d, axis=2),axis=1), bins=np.arange(400)-0.5)[0]
    for k in range(200):
    ...
    return H/N_events, G/N_events, G_spectrum
