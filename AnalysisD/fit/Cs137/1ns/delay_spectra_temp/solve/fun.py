...
def Sim(t, N, F, Tf, Ts, R, b, Q, T, St):
    ...
    for k in range(200):
        ...
        for j in range(len(Q)):
            spectra[:,j]=np.histogram(np.sum(d[:,:100,j], axis=1), bins=np.arange(351)-0.5)[0]
            H[:,k,j]=np.histogram(d[:,k,j], bins=np.arange(np.shape(H)[0]+1)-0.5)[0]
    return H/N_events, G/N_events, spectrum, spectra
...
def Recomb(t, F, Tf, Ts, T, St,b):
    A1=1/Tf*np.exp(-t/Tf)-b/(Tf*(b+t))+b/Tf**2*np.exp(-(t+b)/Tf)*(expi((t+b)/Tf)-expi(b/Tf))
    A2=1/Ts*np.exp(-t/Ts)-b/(Ts*(b+t))+b/Ts**2*np.exp(-(t+b)/Ts)*(expi((t+b)/Ts)-expi(b/Ts))
    I=np.arange(len(t)*len(T))
    B=np.exp(-0.5*(t[I//len(T)]-T[I%len(T)])**2/St[I%len(T)]**2).reshape(len(t), len(T))
    C=convolve2d(B, (F*A1/np.sum(A1)+(1-F)*A2/np.sum(A2)).reshape(len(A1), 1), mode='full')
    return np.ravel(C[:len(t),:]/np.sum(C, axis=0))

def make_3D(t, N, F, Tf, Ts, R, b, Q, T, St):
    dt=t[1]-t[0]
    ...
    B=binom.pmf(nu[I//(len(Q)*len(t)*len(n))], n[I%len(n)], dS[(I//(len(n)*len(t)))%len(Q)]*Q[(I//(len(n)*len(t)))%len(Q)]*model[(I//len(n))%len(t),(I//(len(n)*len(t)))%len(Q)]).reshape(len(nu), len(Q), len(t), len(n))
    P=np.matmul(B, pois)
