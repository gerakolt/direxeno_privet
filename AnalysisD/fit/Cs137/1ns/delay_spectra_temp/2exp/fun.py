...
def Model(h, N, Q, T, R, F, Tf, Ts, St):
    dt=1
    n=np.arange(100,700)
    j=np.arange(200)
    NJ=np.arange(len(n)*len(j))

    i=np.arange(200)
    NJI=np.arange(len(NJ)*len(i))

    p=np.arange(len(St))
    NJIP=np.arange(len(NJI)*len(p))
    B=binom.pmf(h[NJIP//len(p)%len(i), NJIP%len(p)], Q[NJIP%len(p)]*dt*np.exp(-0.5*(NJIP//len(p)%len(i)-NJIP//len(p)//len(i)%len(j)-T[NJIP%len(p)])**2/St[NJIP%len(p)]**2)/np.sqrt(2*np.pi*St[NJIP%len(p)]**2),
            n[NJIP//len(p)//len(i)//len(j)]).reshape(len(NJI), len(p))

    B0=binom.pmf(h[NJIP//len(p)%len(i), NJIP%len(p)]*0, Q[NJIP%len(p)]*dt*np.exp(-0.5*(NJIP//len(p)%len(i)-NJIP//len(p)//len(i)%len(j)-T[NJIP%len(p)])**2/St[NJIP%len(p)]**2)/np.sqrt(2*np.pi*St[NJIP%len(p)]**2),
            n[NJIP//len(p)//len(i)//len(j)]).reshape(len(NJI), len(p))

    model=F/Tf*np.exp(-j/Tf)+(1-F)/Ts*np.exp(-j/Ts)
    M=np.sum(poisson.pmf(n[NJ//len(j)], N*model[NJ%len(j)]*dt)*np.prod(B, axis=1).reshape(len(i), len(NJ)), axis=0)
    M0=np.sum(poisson.pmf(n[NJ//len(j)], N*model[NJ%len(j)]*dt)*np.prod(B0, axis=1).reshape(len(i), len(NJ)), axis=0)

    P=np.zeros(len(i))
    P[0]=np.sum(np.cumprod(np.append(1,M0[:-1]))*M)
    P[1:]=np.sum(np.cumprod(np.append(1,M0[:-1]))*(1-M0)*M)
    return P
