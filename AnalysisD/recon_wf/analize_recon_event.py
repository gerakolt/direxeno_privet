...
x=np.arange(1000)/5
for i in range(len(pmts)):
    np.ravel(ax)[i].hist(rec['init_wf'][:,i], bins=100, range=[0,100], label='PMT{} init_wf'.format(pmts[i]))
    np.ravel(ax)[i].legend(fontsize=15)
...
plt.figure()
plt.hist(np.sum(np.sum(rec['h'], axis=2), axis=1), bins=100, range=[0,1000])
...
plt.hist(init/full, bins=75, range=[0,0.2])
print(rec['id'][np.logical_and(init/full>0.003, init/full<0.006)])
