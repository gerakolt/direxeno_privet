...
def get_delays(pmts):
    ...
    for pmt in pmts:
        if pmt!=15:
            try:
                data=np.load(path+'delay_hist{}-15.npz'.format(pmt))
                delays.append(data['m'])
            except:
                data=np.load(path+'delay_hist15-{}.npz'.format(pmt))
                delays.append(-data['m'])
        else:
            delays.append(0)
    return delays
...
def Recon_wf(WF, wf_origin, height_cut, dh3_cut, spk_cut, SPE, Init):
    ...
    while len(maxis)>0 and counter<100:
        ...
        for i, maxi in enumerate(maxis):
            if wf[maxi]>-height_cut:
                maxis=np.delete(maxis, 0)
            elif wf[maxi]>-spk_cut and (wf[maxi]-wf[maxi-3])/wf[maxi]>dh3_cut:
                if maxi>len(wf)-5:
                    wf[maxi-3:]=wf[maxi-3]
                else:
            ...
            # if int(wf[maxi])==0:
            #     plt.figure()
            #     plt.plot(wf_origin, 'k.')
            #     plt.plot(wf, 'r.')
            #     plt.plot(maxi, wf[maxi], 'go')
            #     plt.show()
        if len(maxis)==0:
            break
