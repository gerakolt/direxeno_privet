...
plt.figure()
plt.hist(np.sum(np.sum(rec['h'][:,:100,:], axis=2), axis=1),  bins=np.arange(250)*4, alpha=1, label='All events')
...
rec=rec[dn<3*up+18]
plt.hist(np.sum(np.sum(rec['h'][:,:100,:], axis=2), axis=1),  bins=np.arange(250)*4, alpha=1, label='up dn')
print(len(rec))
