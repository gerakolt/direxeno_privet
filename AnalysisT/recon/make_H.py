...
spectrum, binsSpec=np.histogram(np.sum(np.sum(rec['h'][:,:50,:], axis=1), axis=1), bins=np.arange(left, right+1)-0.5)
...
for i, pmt in enumerate(pmts):
    h=rec['h'][:,:,i]
    spectra[:,i], binsspec=np.histogram(np.sum(h[:,:50], axis=1), bins=np.arange(101)-0.5)
    for j in range(50):
