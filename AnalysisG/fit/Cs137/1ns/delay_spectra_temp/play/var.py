...
t=np.arange(100)
n=np.arange(np.shape(m)[0])
for i in range(len(pmts)):
    np.ravel(ax)[i].plot(t, np.sum(m[:,:,i].T*n**2, axis=1)-np.sum(m[:,:,i].T*n, axis=1)**2, 'r.', label='model')
    np.ravel(ax)[i].plot(t, np.sum(s[:,:100,i].T*n**2, axis=1)-np.sum(s[:,:100,i].T*n, axis=1)**2, 'b.', label='model')
