...
import time
import os
from fun import Sim, Sim2, q0_model, make_P, model_area, make_3D, make_spectra, Sim3
import sys
...
m=make_3D(t, N, rec['F'][0], rec['Tf'][0], rec['Ts'][0], rec['R'][0], rec['a'][0], rec['eta'][0], rec['Q'][0], rec['T'][0], rec['St'][0])
s, GS, GS_spectrum, Sspectra, Gtrp, Gsng, GRtrp, GRsng=Sim3(N, rec['F'][0], rec['Tf'][0], rec['Ts'][0], rec['R'][0], rec['a'][0], rec['eta'][0], rec['Q'][0], rec['T'][0], rec['St'][0], PEs)

fig, ax=plt.subplots(2,3)
for i in range(len(pmts)):
    # np.ravel(ax)[i].plot(t, np.sum(H[:,:,i].T*np.arange(np.shape(H)[0]), axis=1)/np.sum(H[:,0,i]), 'ko', label='Data - PMT{}'.format(pmts[i]))
    np.ravel(ax)[i].plot(t, np.sum(m[:,:,i].T*np.arange(np.shape(m)[0]), axis=1), 'r.-', label='2 exp model, {}'.format(np.sum(np.sum(m[:,:,i].T*np.arange(np.shape(m)[0]), axis=1))), linewidth=3)
