...
PMTs=[0,1,4,7,8,14]
def Sim(NGlob, Q, T, Strig, R, F, Tf, Ts, St):
    ...
    for i in range(N_events):
        ...
        pmts, n=np.unique(whichPMT(costheta, phi, pmt_mid, pmt_r, pmt_up), return_counts=True)
        n=n[np.isin(pmts, PMTs)]
        pmts=pmts[np.isin(pmts, PMTs)]
