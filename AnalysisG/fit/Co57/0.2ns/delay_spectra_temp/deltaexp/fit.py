...
m=Model(rec['NQ'][0], rec['T'][0],  rec['R'][0], 0, 1000, rec['Ts'][0], rec['St'][0])
s, GS=Sim(rec['NQ'][0], rec['T'][0], 5, rec['R'][0], 0, 1000, rec['Ts'][0], rec['St'][0])
x=np.arange(1000)/5
fig, ax=plt.subplots(2,3)
for i in range(len(pmts)):
    np.ravel(ax)[i].plot(x, np.sum(H[:,:,i].T*np.arange(np.shape(H)[0]), axis=1), 'ko', label='Data - PMT{}'.format(pmts[i]))
    np.ravel(ax)[i].plot(x, np.sum(H[:,0,i])*np.sum(m[:,:,i].T*np.arange(np.shape(m)[0]), axis=1), 'r.-', label='2 exp model', linewidth=3)
    np.ravel(ax)[i].plot(x, np.sum(H[:,0,i])*np.sum(s[:,:,i].T*np.arange(np.shape(s)[0]), axis=1), 'g.-', label='2 exp model', linewidth=3)
