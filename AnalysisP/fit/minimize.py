...
def make_ps(p):
    ...
    ps[:,18:24]=np.random.uniform(0.25, 1, size=len(np.ravel(ps[:,:6]))).reshape(np.shape(ps[:,:6]))
    ps[:, 24]=np.random.uniform(0.001, 2, size=len(ps[:, 18]))
    ps[:, 25]=np.random.uniform(12.5, 15, size=len(ps[:, 18]))
    ps[:, 26]=np.random.uniform(0.001, 1, size=len(ps[:, 18]))
    ps[:, 27]=np.random.uniform(0.5, 1.5, size=len(ps[:, 18]))
    ps[:, 28]=np.random.uniform(15, 50, size=len(ps[:, 18]))
    ps[:, 29]=np.random.uniform(0.35, 0.75, size=len(ps[:, 18]))
    ps[:, 30]=np.random.uniform(0.1,0.2, size=len(ps[:, 18]))
    return ps
