...
# @profile
def Sim_fit(x1, x2, left, right, gamma, Q, T, St, Sa, mu, W, F, Tf, Ts, R, a, PEs):
    N_events=10000
    ...
    d=p.map(make_d, make_iter(N, Q, T, St, Sa, F, Tf, Ts, R, a, v))
    p.close()
    p.join()
    ...
    return H/N_events, spectra/N_events, cov/N_events
...
def make_d(iter_array):
    ...
    for j in range(len(Q)):
        ...
        d[h>0,j]+=np.round(As).astype(int)
        if np.any(h>0):
