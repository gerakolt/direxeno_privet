...
pmts=[0,1,4,7,8,14]
note='Only spectrum, one gamma, variation in w, bins of 20PEs, with Sa'
...
counter=0
ls=[]
ps=np.zeros((15000, 4*len(pmts)+7))
t0=time.time()
...
def L(p):
    ...
    A=multiprocessing.Process(target=make_l, args=(la, H, spectra, cov, left, right, Xcov, 0, 1, gamma, Q[:], T[:], St[:], Sa[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], N))
    ...
    if counter%1==0:
        print(counter, l)
        np.savez(source+'', ps=ps, ls=ls, T=time.time()-t0, note=note)
    return l



def make_l(l, H, spectra, cov, left, right, Xcov, x1, x2, gamma, Q, T, St, Sa, mu, W, F, Tf, Ts, R, a, N):
    S, Sspectra, Scov=Sim_fit(x1, x2, left, right, gamma, Q, T, St, Sa, mu, W, F, Tf, Ts, R, a, np.arange(np.shape(spectra)[0]))
    if np.any(S<0):
        print('S<0', S[S<0])
