...
for i in range(len(pmts)-1):
    for j in range(i+1, len(pmts)):
        ...
        cov[:,k], bins=np.histogram(c, bins=11, range=[-0.1, 0.1])
        k+=1

np.savez(path+'H', H=H, G=G, left=left, right=right, spectra=spectra, spectrum=spectrum, up_dn_cut='dn<3*up+18', cov=cov, Xcov=0.5*(bins[1:]+bins[:-1]))
