...
def Sim(NQ, T, Strig, R, F, Tf, Ts, St, q0, a0, Spad, Spe, m_pad):
    N_events=10000
    d=np.zeros((10000, 500, len(NQ)))
    H=np.zeros((15, 500, len(NQ)))
    for i in range(N_events):
        t0=np.zeros(len(NQ))
        trig=np.random.normal(0, Strig*5/2, 1)
        for j in range(len(NQ)):
            n=np.random.poisson(NQ[j])
            ch0=np.random.choice(15, size=500, replace=True, p=np.append(1-q0[j]/(1-q0[j]), q0[j]**np.arange(1, 15)))
            ...
            td=np.random.normal(trig+5*T[j]/2, St[j]*5/2, nd)
            tf=np.random.normal(trig+5*T[j]/2+np.random.exponential(Tf*5/2, nf), St[j]*5/2, nf)
            ts=np.random.normal(trig+5*T[j]/2+np.random.exponential(Ts*5/2, ns), St[j]*5/2, ns)
            t=np.append(td, np.append(tf, ts))
            h, bins=np.histogram(t, bins=np.arange(501)-0.5)
...
def Model(NQ, T, R, F, Tf, Ts, St, q0, P):
    n=15
    t=np.arange(1000)/5*2
    dt=t[1]-t[0]
    ...
    for i in range(len(NQ)):
        ...
        Ms[:,:,i]=np.matmul(P[:np.shape(Ms)[0],:np.shape(Ms)[0],i], Ms[:,:,i])
        Ms[0,:,i]=(1-q0[i]/(1-q0[i]))*Ms[0,:,i]
        for nu in range(1, np.shape(Ms)[0]):
            Ms[nu,:,i]=(1-q0[i]/(1-q0[i]))*Ms[nu,:,i]+np.sum(q0[i]**(np.arange(1, nu+1))*np.flip(Ms[:nu,:,i], axis=0).T, axis=1)
        Ms[:,:,i]=Ms[:,:,i]/np.sum(Ms[:,:,i], axis=0)
    T0, P0=make_T0(Ms[0,:500,:])
...
def SubModel(NQ, T, R, F, S, Tf, Ts, St, q0, P):
    n=15
    t=np.arange(1000)/5*2
    dt=t[1]-t[0]
    I=np.arange(2*n*len(t))
    Ms=np.zeros((2*n, len(t), len(NQ)))
    for i in range(len(NQ)):
        m=NQ[i]*((1-R[i])*(F*Int(t, Tf, T[i], St[i])+S*Int(t, Ts, T[i], St[i]))+R[i]*dt*np.exp(-0.5*(t-T[i])**2/St[i]**2)/(np.sqrt(2*np.pi)*St[i]))
        Ms[:,:,i]=(poisson.pmf(np.floor(I/len(t)), m[I%len(t)]).reshape((2*n, len(t))))
        Ms[:,:,i]=np.matmul(P[:np.shape(Ms)[0],:np.shape(Ms)[0],i], Ms[:,:,i])
        Ms[0,:,i]=(1-q0[i]/(1-q0[i]))*Ms[0,:,i]
        for nu in range(1, np.shape(Ms)[0]):
