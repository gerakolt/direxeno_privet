...
ax1.plot(x[:30*5], np.sum(H[:,:,0].T*np.arange(np.shape(H)[0]), axis=1)[:30*5], 'k.-', label='Data - PMT7', linewidth=7)
...
ax1.plot(x[:30*5], np.sum(H[:,:,1].T*np.arange(np.shape(H)[0]), axis=1)[:30*5], 'r.-', label='Data - PMT8', linewidth=7)
...
fig.text(0.04, 0.5, r'$N_{events}\sum_n nD_{ni}$', va='center', rotation='vertical', fontsize=25)
ax1.set_xlabel('Time [ns]', fontsize=25)
ax1.tick_params(axis='both', which='major', labelsize=22)

plt.figure()
plt.plot(x[:30*5], np.sum(G[:,:].T*np.arange(np.shape(G)[0]), axis=1)[:30*5], 'k.-', label='Data - PMT7+PMT8', linewidth=7)
