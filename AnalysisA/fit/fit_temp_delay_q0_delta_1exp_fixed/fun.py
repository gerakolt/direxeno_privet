...
def Sim(NQ, T, Strig, R, Ts, St, q0):
    ...
    for i in range(N_events):
        ...
        for j in range(len(NQ)):
            ...
            ch=np.random.choice(2, size=n, replace=True, p=[R[j], (1-R[j])])
...
def Model(NQ, T, R, Ts, St, q0):
    ...
    for i in range(len(NQ)):
        m=NQ[i]*((1-R[i])*(Int(t, Ts, T[i], St[i]))+R[i]*dt*np.exp(-0.5*(t-T[i])**2/St[i]**2)/(np.sqrt(2*np.pi)*St[i]))
