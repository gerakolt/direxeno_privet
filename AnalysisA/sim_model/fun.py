...
def Model(NQ, T, R, F, Tf, Ts, St, q0, P):
    ...
    for i in range(len(NQ)):
        m1=NQ[i]*R[i]*dt*np.exp(-0.5*(t-T[i])**2/St[i]**2)/(np.sqrt(2*np.pi)*St[i])
        m2=NQ[i]*(1-R[i])*F*Int(t, Tf, T[i], St[i])
        m3=NQ[i]*(1-R[i])*(1-F)*Int(t, Ts, T[i], St[i])
        ...
        if np.any(np.sum(Ms[:,:,i], axis=0)==0):
            plt.figure()
            plt.plot(np.sum(Ms[:,:,i], axis=0), 'k.')
            plt.plot(m, 'r.')
            plt.plot(m1, 'go')
            plt.plot(m2, 'bo')
            plt.plot(m3, 'yo')
            plt.show()
            print('np.sum(Ms[:,:,i], axis=0)==0')
            print(np.nonzero(np.sum(Ms[:,:,i], axis=0)==0)[0])
            sys.exit()
