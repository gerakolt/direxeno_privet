...
pmts=[0,1,4,7,8,14]
note='Fitting each direction individialy, only spectra (with mu), 5 PEs per bin, two gammas, range of w'
...
counter=0
ls=[]
ps=np.zeros((15000, 3*len(pmts)+8))
t0=time.time()
...
def L(p):
    ...
    B=multiprocessing.Process(target=make_l, args=(lb, HB, spectraB, covB, leftB, rightB, XcovB, 1, 0, gamma, Q[:], T[:], St[:], mu[0], W[0], g[0], F[0], Tf[0], Ts[0], R[0], a[0]))
...
def make_l(l, H, spectra, cov, left, right, Xcov, x1, x2, gamma, Q, T, St, mu, W, g, F, Tf, Ts, R, a):
    S, Sspectra, Scov=Sim_fit(x1, x2, left, right, gamma, Q, T, St, mu, W, g, F, Tf, Ts, R, a, np.arange(np.shape(spectra)[0]))
    if np.any(S<0):
        print('S<0', S[S<0])
