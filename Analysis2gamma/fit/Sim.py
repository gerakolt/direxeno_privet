...
gamma1=122
gamma2=136
def Sim_fit(x1, x2, left, right, gamma, Q, T, St, mu, W, g, F, Tf, Ts, R, a, PEs):
    N_events=10000
...
def make_d_show(iter_array):
    ...
    t[:recomb]+=5/a*(u/(1-u))
    ch=np.random.choice(2, size=recomb+ex, replace=True, p=[F, 1-F])
    t[ch==0]+=np.random.exponential(Tf, len(t[ch==0]))*5
    t[ch==1]+=np.random.exponential(Ts, len(t[ch==1]))*5
    slow_i=np.nonzero(ch==1)[0]
    fast_i=np.nonzero(ch==0)[0]
    ...
    for j in range(len(Q)):
        hits=np.nonzero(pmt_hit==j)[0]
        ind=np.nonzero(1==np.random.choice(2, size=len(hits), replace=True, p=[1-Q[j], Q[j]]))[0]
        tj=np.random.normal(5*trig+5*T[j]+t[hits[ind]], 5*St[j], len(ind))
        h, bins=np.histogram(tj, bins=np.arange(1001))
        d[0,:,j]=h
        d[1,:,j]=np.histogram(tj[np.nonzero(np.logical_and(hits[ind]>=recomb, np.isin(hits[ind], slow_i)))[0]], bins=np.arange(1001))[0]
        d[2,:,j]=np.histogram(tj[np.nonzero(np.logical_and(hits[ind]>=recomb, np.isin(hits[ind], fast_i)))[0]], bins=np.arange(1001))[0]
        d[3,:,j]=np.histogram(tj[np.nonzero(np.logical_and(hits[ind]<recomb, np.isin(hits[ind], slow_i)))[0]], bins=np.arange(1001))[0]
        d[4,:,j]=np.histogram(tj[np.nonzero(np.logical_and(hits[ind]<recomb, np.isin(hits[ind], fast_i)))[0]], bins=np.arange(1001))[0]
        if np.any(h>0):
            t0[j]=np.amin(np.nonzero(h>0)[0])
    for i in range(5):
        for j in range(len(Q)):
...
def Sim_show(x1, x2, left, right, gamma, Q, T, St, mu, W, g, F, Tf, Ts, R, a, PEs):

    N_events=10000
    H=np.zeros((50, 1000, len(Q)))
    G=np.zeros((250,1000))
    Gtrp=np.zeros((250,1000))
    Gsng=np.zeros((250,1000))
    GRtrp=np.zeros((250,1000))
    GRsng=np.zeros((250,1000))
    spectra=np.zeros((len(PEs),len(Q)))
    cov=np.zeros((11, 15))
    ...
    ds=np.array(ds)
    s=np.sum(np.sum(ds[:,0,:500],axis=1), axis=1)
    ind=np.nonzero(np.logical_and(s>left, s<right))[0]
    if len(ind)==0:
        w=np.abs(np.mean(s)-(left+right)/2)
    ...
    for i in range(len(Q)):
        spectra[:,i]=np.histogram(np.sum(d[:,:500,i], axis=1), bins=np.arange(len(PEs)+1)-0.5)[0]
