...
#     for j in range(len(NQ)):
#         for k in range(200):
#             H[:,k,j]=np.histogram(np.sum(d[:,5*k:5*(k+1),j], axis=1), bins=np.arange(26)-0.5)[0]
#     return H/N_events
...
def Sim(NQ, T, Strig, R, F, Tf, Ts, St):
    ...
    for j in range(len(NQ)):
        for k in range(200):
            H[:,k,j]=np.histogram(d[:,k,j], bins=np.arange(26)-0.5)[0]
    return H/N_events
...
def Global_Sim(NQ, T, Strig, R, F, Tf, Ts, St):
    ...
    for i in range(N_events):
        ...
        for j in range(len(NQ)):
            d[i,:,j]=np.roll(d[i,:,j], -int(np.amin(t0)))
    for k in range(200):
        G[:,k]=np.histogram(np.sum(np.sum(d[:,5*k:5*(k+1),:], axis=2), axis=1), bins=np.arange(26)-0.5)[0]
    return G/N_events
...
def subGlobal_Sim(NQ, T, Strig, R, F, f, Tf, Ts, St):
    ...
    for i in range(N_events):
        ...
        for j in range(len(NQ)):
            n=np.random.poisson(NQ[j])
            ch=np.random.choice(4, size=n, replace=True, p=[R[j], (1-R[j])*F, (1-R[j])*f, 1-R[j]-(1-R[j])*F-(1-R[j])*f])
            nd=len(np.nonzero(ch==0)[0])
            nf=len(np.nonzero(ch==1)[0])
            ns=len(np.nonzero(ch==2)[0])
            td=np.random.normal(trig+5*T[j], St[j]*5, nd)
...
def Const(tau,T,s):
    C=(1-erf(s/(np.sqrt(2)*tau)+T/(np.sqrt(2)*s))+np.exp(-s**2/(2*tau**2)-T/tau)*(1+erf(T/(np.sqrt(2)*s))))
    return 1/C
