...
for j in range(200):
    G[:,j]=np.histogram(np.sum(np.sum(rec['h'][:,j*5:5*(j+1),:], axis=2), axis=1), bins=np.arange(np.shape(G)[0]+1)-0.5)[0]
...
for i, pmt in enumerate(pmts):
    ...
    for j in range(200):
        ...
        print(i,j)
np.savez(path+'H1ns', H=H, G=G, left=left, right=right, spectra=spectra, spectrum=spectrum)
