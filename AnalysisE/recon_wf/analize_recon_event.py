...
x=np.arange(1000)/5
for i in range(len(pmts)):
    np.ravel(ax)[i].hist(rec['init_wf'][:,i], bins=100, range=[0,500], label='PMT{} init_wf'.format(pmts[i]))
    np.ravel(ax)[i].legend(fontsize=15)
...
plt.figure()
plt.hist(np.sum(np.sum(rec['h'][:,:100,:], axis=2), axis=1), bins=100, range=[0,1100])
...
plt.hist(init/full, bins=75, range=[0,1])
...
fig.suptitle('Co57', fontsize=25)
for i in range(len(pmts)):
    np.ravel(ax)[i].hist(np.sum(rec['h'][:,:100,i], axis=1), bins=100, label='PMT{}'.format(pmts[i]))
    np.ravel(ax)[i].legend(fontsize=15)
