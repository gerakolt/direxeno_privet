...
m=Model(rec['NQ'][0], rec['T'][0],  rec['R'][0], 0, 1000, rec['Ts'][0], rec['St'][0])
s, GS, G_spec=Sim(rec['NQ'][0], rec['T'][0], 1e-6, rec['R'][0], 0, 1000, rec['Ts'][0], rec['St'][0])
...
# fig, ax=plt.subplots(2,3)
# for i in range(len(pmts)):
#     np.ravel(ax)[i].step(areas[i], H_areas[i], label='Area spectrum - PMT{}'.format(pmts[i]))
#     np.ravel(ax)[i].legend()
...
ax2.step(np.arange(len(spectrum)), spectrum, label='Spectrum')
ax2.step(np.arange(len(G_spec)), G_spec/np.amax(G_spec)*np.amax(spectrum), label='Sim spectrum')

model=poisson.pmf(np.arange(len(spectrum)), np.sum(GS.T*np.arange(np.shape(GS)[0])))
model=model/np.amax(model)*np.amax(spectrum)
ax2.plot(np.arange(len(spectrum))[left:right], model[left:right], 'r-.')
ax2.legend()
