...
def make_ps():
    ...
    ps[:,6]=np.random.uniform(1.45, 1.7, size=len(np.ravel(ps[:,6]))).reshape(np.shape(ps[:,6]))
    ps[:,7]=np.random.uniform(0.01, 2, size=len(np.ravel(ps[:,6]))).reshape(np.shape(ps[:,6]))
    ps[:,8]=np.random.uniform(13, 25, size=len(np.ravel(ps[:,6]))).reshape(np.shape(ps[:,6]))
    return ps
