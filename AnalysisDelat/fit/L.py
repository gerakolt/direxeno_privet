...
def L(p):
    ...
    if np.any(np.array(dl)[:]>1):
        print('dl', dl[:])
        return 1e10*np.amax(dl)

    la=multiprocessing.Value('d', 0.0)
    lb=multiprocessing.Value('d', 0.0)

    B=multiprocessing.Process(target=make_l, args=(lb, HB, spectraB, covB, leftB, rightB, XcovB, 1, 0, gamma, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], dl[0]))
    A=multiprocessing.Process(target=make_l, args=(la, H, spectra, cov, left, right, Xcov, 0, 1, gamma, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], dl[0]))
...
def make_l(l, H, spectra, cov, left, right, Xcov, x1, x2, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, dl):
    S, Sspectra, Scov=Sim_fit(x1, x2, left, right, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, dl, np.arange(np.shape(spectra)[0]))
    if np.any(S<0):
