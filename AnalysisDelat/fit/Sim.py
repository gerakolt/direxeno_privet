...
# @profile
def Sim_fit(x1, x2, left, right, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, dl, PEs):
    N_events=10000
    ...
    d=p.map(make_d, make_iter(N, Q, T, St, F, Tf, Ts, R, a, v, dl))
    p.close()
    p.join()
...
def make_d_show(iter_array):
    ...
    for j in range(len(Q)):
        ...
        d[2,:,j]=np.histogram(tj[np.logical_and(hits[ind]>=sr+recomb, np.isin(hits[ind], slow_i))], bins=np.arange(201))[0]
        d[3,:,j]=np.histogram(tj[np.logical_and(hits[ind]>=sr+recomb, np.isin(hits[ind], fast_i))], bins=np.arange(201))[0]
        d[4,:,j]=np.histogram(tj[np.logical_and(np.logical_and(hits[ind]>=sr, hits[ind]<sr+recomb), np.isin(hits[ind], slow_i))], bins=np.arange(201))[0]
        d[5,:,j]=np.histogram(tj[np.logical_and(np.logical_and(hits[ind]>=sr, hits[ind]<sr+recomb), np.isin(hits[ind], fast_i))], bins=np.arange(201))[0]
...
def Sim_show(x1, x2, left, right, gamma, Q, T, St, mu, W, F, Tf, Ts, R, a, dl, PEs):

    N_events=10000
    H=np.zeros((50, 200, len(Q)))
    G=np.zeros((250,200))
    Gdl=np.zeros((250,200))
    ...
    N=np.random.poisson(1000*gamma/W, N_events)
    p=multiprocessing.Pool(processes=2)
    ds=p.map(make_d_show, make_iter(N, Q, T, St, F, Tf, Ts, R, a, v, dl))
    p.close()
    p.join()
    ...
    for k in range(200):
        ...
        Gdl[:,k]=np.histogram(np.sum(delta[:,k,:], axis=1), bins=np.arange(np.shape(G)[0]+1)-0.5)[0]
        ...
        for j in range(len(Q)):
            H[:,k,j]=np.histogram(d[:,k,j], bins=np.arange(np.shape(H)[0]+1)-0.5)[0]
    return H/len(ind), spectra/len(ind), cov/len(ind),  G/len(ind), Gtrp/len(ind), Gsng/len(ind), GRtrp/len(ind), GRsng/len(ind), Gdl/len(ind), len(ind)
