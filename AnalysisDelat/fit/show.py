...
p=[0.19545177,  0.15378296,  0.12142937,  0.16684189,  0.17957973,  0.300443 ,
 44.44118856, 47.39063356, 45.02554795, 44.39396908, 43.45564933, 44.77342169,
  1.91220653,  2.69814642,  1.62360807,  2.08119476,  1.25093584,  2.0410435,
  5.53913588, 12.8585211,   0.18309659, 11.88353756, 39.7164813,   0.47549843,
  0.81982705,  0.2378067 ]
...
# print(L(np.array(p)))
S, Sspectra, Scov, SG, Gtrp, Gsng, GRtrp, GRsng, Gdl, SN=Sim_show(x1, x2, left, right, gamma, Q[:], T[:], St[:], mu[0], W[0], F[0], Tf[0], Ts[0], R[0], a[0], dl[0], PEs)
...
plt.figure()
plt.step(t, np.sum(G.T*np.arange(np.shape(G)[0]), axis=1), where='mid')
plt.plot(t, np.sum(G[:,0])*np.sum(SG.T*np.arange(np.shape(SG)[0]), axis=1), 'r.-', label='')
plt.plot(t, np.sum(G[:,0])*np.sum(Gdl.T*np.arange(np.shape(SG)[0]), axis=1), 'm.-', label='')
