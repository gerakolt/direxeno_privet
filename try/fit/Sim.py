...
def Sim_fit(x1, x2, left, right, gamma, Q, W, Sa, g, Bins, bins):
    ...
    N136=N_events-N122
    N=np.append(np.random.poisson(1000*122/W, N122), np.random.poisson(1000*136/W, N136))
...
def make_d(iter):
    ...
    for j in range(len(Q)):
        area=np.sum(np.random.normal(1, Sa[j], np.random.binomial(len(np.nonzero(pmt_hit==j)[0]), Q[j])))
        #s[j]=np.random.binomial(len(np.nonzero(pmt_hit==j)[0]), Q[j])
        if area>0:
            s[j]=int(np.round(area))
    return s
